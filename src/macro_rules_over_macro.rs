...
impl EarlyLintPass for MacroRulesOverMacro {
    fn check_mac_def(&mut self, ctx: &EarlyContext<'_>, def: &ast::MacroDef, _: ast::NodeId) {
        // Don't lint against `macro_rules!` definitions generated by external
        // macros
        if in_external_macro(ctx.sess, def.body.span().unwrap_or_default()) {
            return;
        }

        if def.macro_rules && ctx.sess.features_untracked().decl_macro {
            span_lint_and_help(
                ctx,
